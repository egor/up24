...
$isOrder = $db->fetchRow("SELECT * FROM `tickets` WHERE `code` = '$orderId'");
...
$cost = (float) $db->fetchOne("SELECT `cost` FROM `events`, `event_types` WHERE `events`.`type_id` = `event_types`.`id` AND `events`.`id` = '".$isOrder['event_id']."'");
...
$db->update('tickets', $data, "`id` = '".$isOrder['id']."'");
